...
class AuthScreen(MDScreen):
    ...
    def _authenticate(self, action):
        ...
        if not email or not password:
            self.ids.status.text = tr("Inserisci email e password")
            return
        ...
        def done(_result):
            ...
            toast(tr("Accesso eseguito ({user})").format(user=app.sync.current_user()))
            self.manager.current = "trips"

            def after_sync(_):
                self.manager.get_screen("trips").refresh()
                if offline_pending:
                    toast(tr("I tuoi viaggi offline sono stati caricati sul tuo account"))
        ...
        def failed(exc):
            ...
            self.ids.status.text = str(exc) or tr("Connessione non riuscita")

        self.ids.status.text = tr("Connessione…")
